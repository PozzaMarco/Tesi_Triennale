...
\begin{itemize}
	...
	\end{itemize} 
\end{itemize}
Athesys S.r.l. (il cui logo è rappresentato in Figura 1.1) lavora presso importanti realtà aziendali, operanti nel settore bancario, pubblico e automotive.\\
...
In un modello centralizzato, le organizzazioni stabiliscono una relazione di fiducia \textit{punti a punto} con ogni utente che interagisce con il sistema (vedi Figura 1.3).
...
In un modello decentralizzato abbiamo un'entità terza che attesta e certifica la validità di un'identità auto-generata (vedi Figura 1.4).
...
Gli \textit{Identity Custodians} nascono con l'obiettivo di semplificare la gestione e accrescere la disponibilità e protezione delle identità decentralizzate mantenendo, al loro interno, tutti gli attributi di identità necessari (vedi Figura1.5).
...
\end{figure}
\\
La Figura 1.6 rappresenta un tipico scenario di accesso ai servizi di un \textit{service provider} da parte di un utente che possiede un proprio \gls{identityWallet}.
...
\end{itemize}
La Tabella 1.1 specifica meglio gli obiettivi di stage concordati nel piano di lavoro.
...
In Tabella 1.2 si ha un riepilogo delle attività di stage previste.
